# Description
This is a CSharp (C#) 🟪 to integrate event hubs 📡⚡. This is in order to study and practice for az204 certification exam 🤓📚🎓

# Made With
[![C#](https://img.shields.io/badge/c%23-8c39ac?style=for-the-badge&logo=data:image/svg+xml;base64,&logoColor=white&labelColor=000000)]()
[![NetCore & Net Framework](https://img.shields.io/badge/netcore%20&%20NET%20Framework-4f3ac9?style=for-the-badge&logo=dotnet&logoColor=white&labelColor=000000)]()
[![Azure](https://img.shields.io/badge/Azure-2b83cb?style=for-the-badge&logo=data:image/svg+xml;base64,&logoColor=white&labelColor=000000)]()

# Architecture

## System Overview

This project demonstrates a cloud-native event-driven architecture using Azure services for the AZ-204 certification preparation.

```mermaid
graph LR
    A[C# Producer<br/>EventHubsConsole] -->|Send Events| B[Azure Event Hub<br/>rgeventhub]
    B -->|Trigger| C[Azure Logic Apps<br/>Event Processor]
    C -->|Notification| D[Outlook<br/>Email]
    
    style A fill:#68217a,stroke:#fff,stroke-width:2px,color:#fff
    style B fill:#0078d4,stroke:#fff,stroke-width:2px,color:#fff
    style C fill:#0078d4,stroke:#fff,stroke-width:2px,color:#fff
    style D fill:#0078d4,stroke:#fff,stroke-width:2px,color:#fff
```

## Components

### 1. **C# Producer Application** 🟪
- **Technology**: .NET 8.0 Console Application
- **Purpose**: Generates and sends events to Azure Event Hub
- **Library**: `Azure.Messaging.EventHubs`

### 2. **Azure Event Hub** 📡
- **Service**: Azure Event Hubs
- **Purpose**: Highly scalable real-time event ingestion service
- **Capacity**: Millions of events per second

### 3. **Azure Logic Apps** ⚡
- **Service**: Azure Logic Apps
- **Purpose**: Event orchestration and processing
- **Trigger**: Event Hub trigger for automatic processing

### 4. **Outlook Notification** 📧
- **Service**: Microsoft Outlook
- **Purpose**: Email notification delivery
- **Integration**: Logic Apps native connector

## Data Flow

1. The C# application creates an event batch
2. Events are sent to Azure Event Hub through a secure connection
3. Azure Logic Apps automatically detects new events
4. Logic Apps processes and sends notifications to Outlook
5. Recipients receive emails with event information